...
quantity operator-(quantity<T, D> quant) { return quantity<T, D>(q - quant.q); }
```
위 `operator+`는 인자로 받는 `quantity`의 데이터 타입과 `Dim` 타입이 일치해야지만 인스턴스화 된다.
만약, 데이터 타입이나 `Dim` 타입이 일치하지 않았더라면 저 `operator+`는 인스턴스화 될 수 없고 따라서 컴파일러는 저 `operator+`를 찾을 수 없다는 오류를 발생시킨다.
...
```
즉 위 `+`에 해당하는 연산자 함수를 찾을 수 없다는 것이다.

예상했던 대로, 위 부분에서 오류가 발생하는데, `kg`과 `meter` 의 단위가 다르기 때문이다. 반면,
```cpp
kg + kg;
```
은 잘 컴파일 된다.

그렇다면 이제 `*`와 `/` 연산자만 만들어주면 된다. 하지만 `*`와 `/`의 경우 `+`와 `-`보다 좀 더 까다롭다. 왜냐하면 `*`와 `/`의 경우 굳이 `Dim`이 일치하지 않아도 되기 때문이다. 다만 이 연산을 수행했을 때 새로운 차원의 데이터가 나올 뿐이다.

예를 들어, 가속도를 나타내기 위해서는
```cpp
meter / (second * second)
```
이렇게 해주면 된다. 다만 새로운 타원의 데이터(`Dim<zero, one, minus_two>`)가 탄생할 뿐이다.
따라서, `operator*`와 `operator/`의 경우 두 개의 다른 차원의 값을 받아도 처리할 수 있어야 한다. 따라서 `operator*`와 `/`를 정의해보면 다음과 같다.
```cpp
...
```
새로 만들어지는 타입의 차원은 당연히 `add_dim_<D, D2>::type`이 될 것이고(`operator*` 의 경우), 그 값은 그냥 실제 값을 곱해주면 된다. 이와 더불어
```cpp
3 * kg
```
과 같은 곱도 처리해야 하기 때문에, 아래와 같은 함수들도 정의해주어야 한다.
```cpp
quantity<T, D> operator*(T scalar) { return quantity<T, D>(q * scalar); }
quantity<T, D> operator/(T scalar) { return quantity<T, D>(q / scalar); }
```
이는 위처럼 일반적인 차원이 없는 값과의 곱도 지원해준다. 그렇다면 예를 들어, 아래와 같이 정의된 `F`의 타입은 어떨까?
```cpp
F = kg * meter / (second * second);
```
일단 `F`의 차원은 계산해보면 (1, 1, -2)이렇게 나올 것이다. 따라서, `F`의 `dim` 타입은 `<Ratio<1, 1>, Ratio<1, 1>, Ratio<-2, 1>>`이 될 것이다. 다시 말해, `F`를 다음과 같이 나타낼 수 있다.
```cpp
quantity<double, Dim<one, one, Ratio<-2, 1>>> F =
    kg * meter / (second * second);
```
그런데, 매번 변수를 정의할 때마다 저렇게 길고 긴 타입을 써주는 것은 매우 귀찮은 일이다. 저 `kg * meter / (second * second)`를 계산해서 나오는 객체의 타입이 저렇게 된다는 사실은 우리도 알고 컴파일러도 안다. 

그럼 컴파일러가 쉽게 알아낼 수 있는 타입을 굳이 써주어야 할까? 컴파일러가 타입을 알아서 생각하도록 하면 안될까? 물론 가능하다.

## auto 키워드

C++ 코드를 많이 짜면서 느꼈겠지만, 객체를 생성할 때, 많은 경우 굳이 타입을 쓰지 않아도 알아서 추측할 수 있는 경우들이 많이 있다.

예를 들어,
```cpp
(??) a  = 3;
```
과 같이 썼다면 저 (??)는 아마 `int`를 의도한 것일 것이다. 아니면
```cpp
some_class a;
(??) b = a;
```
의 경우 저 (??)에는 아마 `some_class`가 들어갈 것이다. 즉, 객체가 복사 생성될 때, 그 복사 생성하는 대상의 타입을 확실히 알 수 있다면 굳이 그 객체의 타입을 명시하지 않아도 컴파일러가 알아낼 수 있다.

물론 때때로 컴파일러가 타입을 제대로 유추할 수 없는 경우도 있다. 예를 들어,
```cpp
quantity<double, Dim<one, zero, zero>> kg(1);
```
의 경우 만약 저 타입 부분을 가리고
```cpp
(??) kg(1);
```
와 같이 살펴본다면 어떨까? 컴파일러 입장에서는 단순히 생각해봤을 때 그냥 1로 초기화하는 변수이므로 (??)에는 int가 들어갈 것이다. 따라서 이 경우에는 우리가 원하는 타입으로 생성할 수 없다. 반면,
```cpp
(??) F = kg * meter / (second * second);
```
`F`의 경우 우리가 굳이 타입을 적지 않아도 컴파일러가 오른쪽의 연산을 통해서 `F`의 타입을 정확하게 알아낼 수 있다.

이와 같이 컴파일러가 타입을 정확히 알아낼 수 있는 경우 굳이 긴 타입을 적지 않고 간단하 `auto`로 표현할 수 있다. 그리고 그 `auto`에 해당하는 타입은 컴파일 시에 컴파일러에 의해 추론된다.