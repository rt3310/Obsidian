...
통상적으로 자기 자신을 반환하지 않는 이항 연산자들, 예를 들어 위와 같은 **`+`, `-`, `*`, `/`들은 모두 외부 함수로 선언하는 것이 원칙**이다.
반대로 자기 자신을 반환하는 이항 연산자들, 예를 들어 **`+=`, `-=` 같은 애들은 모두 멤버 함수로 선언하는 것이 원칙**이다. 따라서 위 코드를 수정해보면 다음과 같이 할 수 있다.
```cpp
class Test {
private:
	int _value;
